...
const products = [
  {
    ...
    price: "R$ 22,80",
    href: "https://30-receitas-ganho-de-massajj.vercel.app/",
    theme: "",
  },
  {
    tag: "NUTRIÇÃO - E-BOOK",
    title: "30 Receitas para Emagrecer",
    description:
      "Receitas saudáveis, práticas e deliciosas. Feito para quem quer perder peso com saúde e sem abrir mão do sabor.",
    price: "R$ 22,80",
    href: "https://pombaz.vercel.app/receitamagica",
    theme: "pombaz-blue-theme",
  },
]
      ...
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-28 lg:px-0">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 md:gap-5">
          {products.map((item, index) => (
            <Reveal key={index} className="h-full" delay={80 + index * 100}>
              <div className={`${item.theme} h-full`}>
                <div
                  className="pombaz-card group relative flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-[#1b1b1b] bg-[#050505] p-1.5 shadow-2xl shadow-black/50 hover:border-[rgba(var(--brand-yellow-rgb),0.32)]"
                  data-cursor="hover"
                >
                  <div className="relative flex h-full flex-col overflow-hidden rounded-[1.15rem] border border-[#202020] bg-[linear-gradient(135deg,#101010_0%,#080808_42%,#0f0f0f_100%)] p-6 md:p-9">
                    ...
                    <div className="pointer-events-none absolute left-1/2 top-0 h-px w-1/2 -translate-x-1/2 bg-gradient-to-r from-transparent via-[rgba(var(--brand-yellow-rgb),0.8)] to-transparent shadow-[0_0_16px_rgba(var(--brand-yellow-rgb),0.55)]" aria-hidden="true" />
                    <div className="pointer-events-none absolute -right-16 top-0 h-44 w-44 rounded-full bg-[rgba(var(--brand-yellow-rgb),0.06)] blur-[70px]" aria-hidden="true" />
          ...
          {[...Array(3 - products.length)].map((_, item) => (
            <Reveal key={item} className="h-full" delay={180 + (products.length + item) * 100}>
              <div
                aria-hidden="true"
